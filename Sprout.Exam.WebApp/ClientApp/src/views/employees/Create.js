...
export class EmployeeCreate extends Component {
    ...
    async saveEmployee() {
        ...
        const employee = {
            ...
            tin: this.state.tin,
            EmployeeTypeId: parseInt(this.state.typeId)
        };
        ...
        else if (response.status === 409) { // If there's a conflict (TIN already exists)
            this.setState({ loadingSave: false });
            alert("An employee with this TIN already exists.");
        }
        else {
