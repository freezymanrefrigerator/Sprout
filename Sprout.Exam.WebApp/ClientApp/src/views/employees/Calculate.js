...
export class EmployeeCalculate extends Component {
  ...
  handleChange(event) {
    ...
    handleSubmit(e) {
        ...
        let absentDays = parseFloat(this.state.absentDays);
        let workedDays = parseFloat(this.state.workedDays);

        if (this.state.employeeTypeId === 1) {
            if (isNaN(absentDays) || absentDays < 0 || absentDays > 1000) {
                alert('Please enter a valid non-negative value for absent days, less than or equal to 1000.');
                return;
            }
        } else {
            if (isNaN(workedDays) || workedDays <= 0 || workedDays > 1000) {
                alert('Please enter a valid positive value for worked days, less than or equal to 1000.');
                return;
            }
        }
...
<div className="form-row">
<div className='form-group col-md-12'>
<label>Net Income: <b>{parseFloat(this.state.netIncome).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b></label>
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
    ...
    async calculateSalary() {
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({
                id: this.state.id,
                absentDays: parseFloat(this.state.absentDays),
                workedDays: parseFloat(this.state.workedDays)
            })
        };
        ...
        try {
            ...
            alert("There was an error during salary calculation.");
        }
    }
